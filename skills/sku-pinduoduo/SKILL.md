...
2. 若有已批准的 `REFERENCE_MIGRATION_CONTEXT`，读取当前平台迁移条目并把抽象特征映射到拼多多合法槽位；SKU、价格、数量与活动规则覆盖来源页面做法。
3. 建立 SKU—价格—图片一致性矩阵，识别最低价 SKU 与曝光商品的差异。
4. 区分曝光图、轮播图、SKU 图和详情图。
5. 建立或复用 `CREATIVE_CONTEXT`，生成三个差异化方向，以价值确定性、使用证明或规格清晰为核心，不默认做廉价促销风。
6. 确认方向后，按槽位创意自由度选择 F0–F3，并严格按逐屏 / 逐模块生产协议，为每个槽位完整输出 Prompt、Negative Prompt、处理模式、文案位置、后期排版、镜头矩阵、产品一致性质检和通用 Prompt 拦截。
7. 检查品牌、材质、型号、功效、数量和活动标识是否相互一致且有证据。
