...
4. 建立或复用 `CREATIVE_CONTEXT`，从品牌系统与产品差异共同生成三个差异化方向，不用空泛黑金或伪奢华替代品牌证据。
5. 用户确认后，按槽位创意自由度选择 F0–F3，并严格按逐屏 / 逐模块生产协议，为每个槽位完整输出 Prompt、Negative Prompt、处理模式、品牌资产来源、文案位置、后期排版、镜头矩阵、产品一致性质检和通用 Prompt 拦截。
