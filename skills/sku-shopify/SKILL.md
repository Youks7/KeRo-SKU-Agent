...
4. 建立或复用 `CREATIVE_CONTEXT`，提出三个差异化响应式创意方向；关键文字保留为 HTML，图片提供 alt text，CTA 使用真实交互组件。
5. 用户确认方向后，按槽位创意自由度选择 F0–F3，并严格按逐屏 / 逐模块生产协议，为每个含媒体的 PDP Section 完整输出 Prompt、Negative Prompt、处理模式、HTML 文案位置、响应式后期排版、镜头矩阵、产品一致性质检和通用 Prompt 拦截；纯组件也必须保留模板并标记 Prompt 为 N/A。
