...
目标：验证 F2 保留来源图中已有 Logo、标签和包装文字，不再被通用 Negative Prompt 反向删除。
...
创意命题：用真实包装与克制环境建立开箱关系，不改变任何包装证据。
槽位创意自由度：2——只允许受控背景、环境光与接触阴影。
身份锚点：产品轮廓、包装结构、Logo 位置、标签版式和包装文字像素。
参考抽象：仅使用中性使用场景与留白，不引入参考品牌元素。
发布闸门：现有 Logo、标签和包装文字完成人工像素比对前保持 `human-review`。
