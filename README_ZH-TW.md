...
<p align="center">
  <a href="./packages/kero-sku-skills-v1.5-bundle.zip">下載 V1.5 全平台合集</a> ·
...
KeRo SKU Agent 是一個用於 **真實 SKU 事實分析、參考詳情頁遷移、平台路由、商品頁策劃與 AI 視覺生產** 的 Codex 自訂 Agent。它負責編排十一個 Skills，而專業規則仍由 Skills 提供。
...
sku-product-core：產品事實與保真分析
   ↓
sku-reference-migration：參考分段、權利判斷、M1/M2/M3 與模塊映射
   ↓
總路由：平台與素材槽位選擇
   ↓
對應平台專用 Skill
...
V1.5 在鎖定真實 SKU 身分的基礎上，加入參考頁語義分段、M1/M2/M3、證據綁定的模塊映射、三套遷移方向、跨平台重排與斷點續作。
...
完整使用 V1.5 時，下載合集並匯入 `sku-product-core`、`sku-reference-migration` 和需要的平台 Skill：
...
- [參考詳情頁遷移示例](./examples/reference-detail-page-migration.md)
...
目前開發版本：**V1.5.0-dev 參考遷移視覺導演版**

穩定基線版本：**Lite V1.2.1**。V1.5 在多平台與身分合同基礎上增加參考詳情頁遷移、平台化重排和遷移確認門。
